...
Have Reeborg move around the room and close the windows by using the ``build_wall()`` function. The size of the room and the location of the windows are always changing. Note that the green colored square (one step in front of where Reeborg starts) is a goal in this world, so it behaves the same as a flag. You can ask if you are on it using ``at_goal()``.
